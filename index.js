...
var URL_OF_DESIGNER_RE = /^(https?|ftp):\/\//;
...
function svg2ttf(svgString, options) {
  ...
  font.designer = options.designer;

  // TTF Name IDs: https://docs.microsoft.com/en-us/typography/opentype/spec/name#name-ids
  ...
  var designerUrl = options.designerurl || '';

  if (designerUrl !== '' && !URL_OF_DESIGNER_RE.test(designerUrl)) {
    throw new Error('svg2ttf: invalid option, designerUrl must be https://, http:// or ftp:// at the beginning.');
  }
  font.sfntNames.push({ id: 12, value: designerUrl }); // URL Designer

  var licenseDescription = options.license || '';

  font.sfntNames.push({ id: 13, value: licenseDescription }); // License Description

  var licenseUrl = options.licenseurl || '';

  if (licenseUrl !== '' && !URL_OF_DESIGNER_RE.test(licenseUrl)) {
    throw new Error('svg2ttf: invalid option, licenseurl must be https://, http:// or ftp:// at the beginning.');
  }

  font.sfntNames.push({ id: 14, value: licenseUrl }); // License URL
