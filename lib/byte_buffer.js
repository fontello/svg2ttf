...
var ByteBuffer = function (buffer, start, length) {
  ...
  this.setUint16 = function(pos, value, littleEndian) {
    if (littleEndian) {
      this.buffer[pos + this.start] = value & 0xFF;
      this.buffer[pos + 1 + this.start] = (value >>> 8) & 0xFF;
    } else {
      this.buffer[pos + this.start] = (value >>> 8) & 0xFF;
      this.buffer[pos + 1 + this.start] = value & 0xFF;
    }
  };
  ...
  this.setUint32 = function(pos, value, littleEndian) {
    if (littleEndian) {
      this.buffer[pos + this.start] = value & 0xFF;
      this.buffer[pos + 1 + this.start] = (value >>> 8) & 0xFF;
      this.buffer[pos + 2 + this.start] = (value >>> 16) & 0xFF;
      this.buffer[pos + 3 + this.start] = (value >>> 24) & 0xFF;
    } else {
      this.buffer[pos + this.start] = (value >>> 24) & 0xFF;
      this.buffer[pos + 1 + this.start] = (value >>> 16) & 0xFF;
      this.buffer[pos + 2 + this.start] = (value >>> 8) & 0xFF;
      this.buffer[pos + 3 + this.start] = value & 0xFF;
    }
  };
  ...
  this.toString = function(offset, length) {
