...
var fillCMap = require('ttf/cmap.js');
var fillHead = require('ttf/head.js');
...
function writeTableHeader(buf, name, checkSum, offset, length) {
  ...
  utils.writeNum(tableOffset || 0, 4);
...
function createTTF(font) {
  ...
  //add TTF header
  utils.writeNum(buf, CONST.VERSION, 4);
  utils.writeNum(buf, CONST.TABLE_COUNT, 2);
  utils.writeNum(buf, searchRange, 2);
  utils.writeNum(buf, entrySelector, 2);
  utils.writeNum(buf, rangeShift, 2);
  ...
  //add tables
  var tableInfo = [];
  tableInfo.push(fillCMap(font));
  tableInfo.push(fillHead(font));

  //fill headers again with actual info
  var i = 0;
  utils.setPosition(buf, tableHeaderOffset);
  _.forEach(TABLES, function(table) {
    writeTableHeader(buf, table, 0, tableInfo[i].position, tableInfo[i].length);
  });
}
