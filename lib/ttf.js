...
var TTFTables = require("./ttf_tables").TTFTables;
...
var TTF = {
  ...
  _getLength: function (object, schema) {
    var length = 0;
  ...
  _addObject: function (view, value, schema, offset) {
    var length = 0;
    _.forEach(schema, function (property) {
      if (_.isArray(property.value)) {
  ...
  _addValue: function (view, value, length, signed) {
    if (value == undefined)
      value = 0;
    switch (length) {
      case 1:
  ...
  _initData: function (schema) {
    ...
    _.forEach(schema, function (field) {
      ...
      if (_.isArray(field.value)) {
        property = {};
        if (property.isFixed) //single instance of array should be presented
  ...
  _newArray: function (schema) {
    return this._initData(schema);
  },

  _addArrayValue: function (obj, value) {
    if (_.isArray(value))
