...
function interpolatePoints(contours) {
  var prevPoint, prevPrevPoint, prevCorrectedPoint;
  var resContours = [];
  var resContour;
...
function convertToRelativePoints(contours) {
  ...
  _.forEach(contours, function (contour) {
    ...
    _.forEach(contour, function (point) {
      resContour.push({
        x: floatToInt(point.x - prevPoint.x),
        y: floatToInt(point.y - prevPoint.y),
...
function glyphDataLength(glyph) {
  ...
  _.forEach(convertToRelativePoints(interpolatePoints(glyph.contours)), function (contour) {
    _.forEach(contour, function (point) {
