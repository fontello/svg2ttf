...
function getFirstCharIndex(font) {
  var minGlyph = _.min(font.glyphs, function(glyph) {
    return glyph.isMissed ? 0xFFFF : glyph.unicode;
  });
  if (minGlyph) {
    return minGlyph.unicode > 0xFFFF ? 0xFFFF : minGlyph.unicode;
  } else {
    return 0xFFFF;
  }
}
...
function getLastCharIndex(font) {
  ...
  if (maxGlyph) {
    return maxGlyph.unicode > 0xFFFF ? 0xFFFF : maxGlyph.unicode;
  } else {
    return 0xFFFF;
  }
}
...
function createOS2Table(font) {
  ...
  buf.writeInt16(font.width); // xAvgCharWidth
