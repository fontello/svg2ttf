...
function getSegments(font, lBound, uBound) {
  var prevGlyph = null;
  var result = [];
  var segment = {};
  ...
  _.forEach(font.glyphs, function (glyph) {
    if ((lBound === undefined || glyph.unicode >= lBound) && (uBound === undefined || glyph.unicode <= uBound)) {
...
function createCMapTable(font) {
  ...
  var segments2bytes = getSegments(font, 0, 0xFFFF); //get segments for unicodes < 0xFFFF
  var segments4bytes = getSegments(font, 0x10000); //get segments for all unicodes

  var buf = new jDataView(tableSize(glyphs2bytes, segments2bytes, segments4bytes));

  var subTableCount = 2; // CMAP table will always have subtables 0 and 4
  if (segments4bytes.length > 0) { //subtable 12 will be created only if 3-bytes unicodes are presented
    subTableCount++;
  }

  buf.writeUint16(0); // version
  buf.writeUint16(subTableCount); // count
  ...
  subTableOffsets.push(buf.tell());
  var subTable12LengthPosition = 0;

  if (segments4bytes.length > 0) {
    // Add subtable format 12
    buf.writeUint16(12); // format
    buf.writeUint16(0); // reserved
    subTable12LengthPosition = buf.tell();
