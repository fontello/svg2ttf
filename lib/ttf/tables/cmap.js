...
function tableSize(glyphs2bytes, segments2bytes, segments4bytes) {
  ...
  result += segments4bytes ? (segments4bytes.length * 12) : 0; // subtable 12 segments
  return result;
}
...
function createCMapTable(font) {
  ...
  if (segments4bytes) { //subtable 12 will be created only if 3-bytes unicodes are presented
    subTableCount++;
  }
  ...
  for (i = 0; i < 256; i++) {
    buf.writeUint8(unicodes.indexOf(i) >= 0 ? getIDByUnicode(font.glyphs, i) : 0); // existing char in table 0..255
  }
