...
function getBufSize(glyphs2bytes, segments2bytes, segments4bytes) {
  var result = 2 * 2 + 3 * 8 + 262; // table header + subtable declarations + subtable 0 format
  result += 2 * 12; // subtable 4 header and required array elements
  result += segments2bytes.length * 2 * 4; // subtable 4 segments
  result += glyphs2bytes.length * 2; // subtable 4 glyphs
  result += 2 * 2 + 3 * 4; // subtable 12 header
  result += segments4bytes.length * 3 * 4; // subtable 12 segments
  return result;
}
...
function createCMapTable(font) {
  ...
  var bufSize = getBufSize(glyphs2bytes, segments2bytes, segments4bytes);
