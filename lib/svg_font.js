...
var SVG_COORDS_PATTERN = new RegExp('-?[^\\s,-]+', 'g', 'm');
...
//TTF coordinates are always relative, we need to convert absolute coorditanes to relative ones
    function getCoord(font, glyph, path, property, value, convertToRelative) {
        ...
        if (path.length > 0 && convertToRelative)
            result = Math.round(+value - path[path.length - 1][property]);
        else
            result = Math.round(+value);
        return result;
    }

    function addPoint(path, x, y, startNewContour, flags, convertToRelative) {
        var relX, relY;
        if (startNewContour) {
            sumX = 0;
            sumY = 0;
    ...
    function addPoints(font, glyph, path, svgCommandType, point) {
        ...
        var isCurve = svgCommandType.flags & FLAGS.CURVE;
        if (isCurve) {
            //add quadratic Bézier curve
            if (svgCommandType.flags & FLAGS.QUAD_BEZIER || svgCommandType.flags & FLAGS.SMOOTH_QUAD_BEZIER) {
                var x, y;
                if (svgCommandType.flags & FLAGS.QUAD_BEZIER) {
                    addPoint(path, point.other[0], point.other[1], startNewContour, TTF_FLAGS.OFF_CURVE, convertToRelative);
                } else { //get control point from the previous point
                    var prevPoint = path[path.length - 1];
                    if (prevPoint != undefined) {
                        addPoint(path, prevPoint.x, prevPoint.y, startNewContour, TTF_FLAGS.OFF_CURVE, false);
                    }
                }
            }
        }
        addPoint(path, point.x, point.y, startNewContour, TTF_FLAGS.ON_CURVE, convertToRelative);
    }

    function addTTFCommand(font, glyph, path, svgCommandRaw) {
        ...
        _.forEach(coords, function (point) {
            addPoints(font, glyph, path, svgCommandType, point);
        });
    ...
    if (glyph.d) {
        ...
        _.forEach(commands, function (command) {
            addTTFCommand(font, glyph, path, command);
        });
    }
    return path;
}
